...
#hash each word from dictionairy and loop through list, until match with entry is found
def dic_atk(dictionary, hashes):
    passwords = []
    dictionary = dictionary.split() #split dictionary into list of words
    for word in dictionary:
        hashed_word = hash_word(word)
        for hash in hashes:
            if hashed_word == hash:
                passwords.append(word)
                break #stop searching for same hash once hash its found
                
    
    return passwords

print(dic_atk(dictionary, entry))
